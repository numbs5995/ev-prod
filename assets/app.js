...
function freshDB(){return{schema:1,activeEvent:null,events:[],products:[],variants:[],talents:[],vendors:[],lots:[],sales:[],pools:[],bookings:[],packs:[],todos:[],expenses:[],log:[]};}
let DB=null;
function load(){
...
const UI={
 ...
 itemRow(v){
   ...
   <td style="white-space:nowrap"><button class="btn sm ghost" onclick="UI.openVariant('${v.productId}','${v.id}')">Edit</button> <button class="btn sm ghost" onclick="UI.openLot('${v.id}')">+ Lot</button> <button class="btn sm ghost" onclick="UI.cloneVariant('${v.id}')">Clone</button> <button class="btn sm ghost" style="color:var(--danger)" onclick="UI.del('variant','${v.id}')">✕</button></td></tr>`;
  },
 renderItems(){
...
/* ---- bookings ---- */
 renderBookings(){
  ...
  let html=`<div class="twrap"><table class="sheet"><colgroup>
   <col style="width:36px"><col style="width:44px"><col style="width:10%"><col style="width:8%"><col style="width:8%"><col style="width:17%"><col style="width:8%"><col style="width:6%"><col style="width:9%"><col style="width:9%"><col style="width:7%"><col style="width:7%"><col style="width:7%"><col style="width:56px">
  </colgroup><thead><tr>
 ...
 selBooking(id){const s=window.__bookSel;s.has(id)?s.delete(id):s.add(id);render();},
  ...
  renderBookingsOrders(wrap,rows,sel){
   ...
   UI.ordPage=UI.ordPage||1;UI.ordPerPage=UI.ordPerPage||20;
   ...
   <div style="display:flex;align-items:center;gap:14px;padding:14px 6px;flex-wrap:wrap">
    ...
    <select style="width:auto" onchange="UI.ordPerPage=+this.value;UI.ordPage=1;render()">${[10,20,50,100].map(x=>`<option ${x===UI.ordPerPage?'selected':''}>${x}</option>`).join('')}</select>
    <span style="flex:1"></span>
...
render();
if(!IS_BOOKINGS_PAGE&&!IS_ITEMS_PAGE&&!IS_TALENTS_PAGE&&location.hash){const v=location.hash.replace('#/','');if(['dashboard','event','vendors','gacha','bookings','sales','todo','calculator','sync'].includes(v))UI.goto(v);}
