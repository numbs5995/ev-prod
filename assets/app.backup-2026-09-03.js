...
const UI={
 ...
 renderBookings(){
  ...
  document.getElementById('bookFilters').innerHTML=`<div class="filters"><button class="fbtn ${!stage?'on':''}" onclick="UI.bookStage='';render()">All</button>${
   ...
   <div class="viewswitch"><button class="${view==='sheet'?'on':''}" onclick="UI.bookView='sheet';render()">▤ Sheet</button><button class="${view==='orders'?'on':''}" onclick="UI.bookView='orders';render()">☰ Orders</button><button class="${view==='dxgrid'?'on':''}" onclick="UI.bookView='dxgrid';render()">▦ DataGrid</button></div>
  ...
  if(view==='dxgrid')return this.renderBookingsDataGrid(wrap,rows,sel);
  ...
  let html=`<div class="twrap"><table class="sheet"><colgroup>
   <col style="width:36px"><col style="width:44px"><col style="width:11%"><col style="width:9%"><col style="width:11%"><col style="width:20%"><col style="width:9%"><col style="width:7%"><col style="width:8%"><col style="width:6%"><col style="width:6%"><col style="width:6%"><col style="width:8%"><col style="width:56px">
  </colgroup><thead><tr>
   ...
   <th>Order total</th><th style="width:80px">Ship</th><th style="width:86px">Fulfil</th><th style="width:96px">Status</th><th style="width:56px"></th></tr></thead><tbody>`;
  pageRows.forEach((b,bi)=>{
    ...
    html+=`<tr class="ordband" style="--oc:${oc}"${selCls?selCls:''} data-oc>
      ...
      <td style="white-space:nowrap">
        ${b.status==='paid'?`<button class="btn sm" onclick="UI.fulfilBooking('${b.id}')">✓</button>`:''}
    ...
    (b.items&&b.items.length?b.items:[{}]).forEach((i,ix)=>{
      ...
      <td><select class="cellinp" onchange="UI.editBookItem('${b.id}',${ix},'variantId',this.value)">${vs.map(v2=>`<option value="${v2.id}" ${v&&v.id===v2.id?'selected':''}>${esc(variantLabel(v2))}</option>`).join('')}${i.lineNotes?`<small class="mut" style="display:block;font-size:.7rem" title="${esc(i.lineNotes)}">📝 ${esc(i.lineNotes.slice(0,40))}</small>`:''}</td>
      <td><input class="cellinp cellpack" placeholder="+ pkg" value="${esc(i.pack||'')}" onchange="UI.editBookItem('${b.id}',${ix},'pack',this.value)"></td>
 ...
 renderBookingsDataGrid(wrap,rows,sel){
  if(!window.jQuery||!jQuery.fn.dxDataGrid){wrap.innerHTML='<div class="card empty">DevExtreme not loaded.</div>';return;}
  const lineTotal=i=>i.price??(i.qty*(vid2var()[i.variantId]?priceOf(vid2var()[i.variantId]):0));
  const data=[];
  rows.forEach((b,bi)=>{
    const orderTotal=(b.items||[]).reduce((s,x)=>s+(x.price??(x.qty*(vid2var()[x.variantId]?priceOf(vid2var()[x.variantId]):0))),0)+(b.shipFee||0);
    (b.items&&b.items.length?b.items:[{qty:0}]).forEach((i,ix)=>{
      const v=vid2var()[i.variantId];
      data.push({id:b.id+'_'+ix,orderNo:bi+1,customer:b.customer||'-',contact:b.contact||'',address:b.address||'',
        item:v?variantLabel(v):'(unmatched)',pack:i.pack||'',qty:i.qty||0,lineTotal:i.qty?lineTotal(i):0,
        orderTotal,shipFee:b.shipFee||0,fulfil:b.fulfil,status:b.status,notes:b.notes||'',src:b.source||''});});
  });
  wrap.innerHTML='<div id="dxBookGrid"></div>';
  jQuery('#dxBookGrid').dxDataGrid({
    dataSource:data,
    keyExpr:'id',
    showBorders:true,
    ...
    onRowPrepared:e=>{
      if(e.rowType==='group'){
        e.rowElement.css({background:'color-mix(in srgb, var(--accent) 7%, var(--card))',fontWeight:700});
      }
    },
    columns:[
      {dataField:'orderNo',caption:'#',width:60,groupIndex:0,
        groupCellTemplate:(el,info)=>{
          const first=info.items&&info.items[0]||{};
          el.html('<span style="font-family:var(--mono);font-weight:800;color:var(--accent)">Order #'+info.value+'</span>'
            +' <span style="margin-left:10px;font-weight:800">'+esc(first.customer||'')+'</span>'
            +(first.src?' <small style="color:var(--muted)">doc '+esc(first.src)+'</small>':''));
        }},
      {dataField:'customer',caption:'Customer',width:150},
      {dataField:'contact',caption:'Contact / WA',width:140,cellTemplate:(el,o)=>{const c=o.data.contact;if(c)el.html('<a class="walink" href="https://wa.me/'+String(c).replace(/\D/g,'').replace(/^0/,'62')+'" target="_blank" rel="noopener">'+esc(c)+'</a>');else el.text('-');}},
      {dataField:'address',caption:'Address',width:200,cellTemplate:(el,o)=>{el.html('<span style="font-size:.75rem">'+esc(o.data.address||'-')+'</span>');}},
      {dataField:'item',caption:'Item',minWidth:200},
      {dataField:'pack',caption:'Packaging',width:110},
      {dataField:'qty',caption:'Qty',width:70,alignment:'center'},
      {dataField:'lineTotal',caption:'Line total',width:110,alignment:'right',calculateDisplayCellValue:r=>r.lineTotal?rp(r.lineTotal):''},
      {dataField:'orderTotal',caption:'Order total',width:120,alignment:'right',visible:false,
        groupCellTemplate:(el,info)=>{
          const d=info.items&&info.items[0]||{};
          el.html('<b style="color:var(--accent);font-family:var(--mono)">Rp'+((d.orderTotal)||0).toLocaleString('id-ID')+'</b>'+(d.shipFee?' <small style="color:var(--muted)">+ship</small>':''));
        }},
      {dataField:'fulfil',caption:'Fulfil',width:80},
      {dataField:'status',caption:'Status',width:100,cellTemplate:(el,o)=>{const s=o.data.status;const cls=s==='paid'||s==='fulfilled'||s==='shipped'?'ok':s==='cancelled'?'bad':'warn';el.html('<span class="status-pill '+cls+'" style="font-size:.7rem;padding:4px 10px;min-width:0">'+esc(s)+'</span>');}},
      {dataField:'notes',caption:'Notes',visible:false},
      {dataField:'src',caption:'Source',visible:false}
    ]
  });
 },
